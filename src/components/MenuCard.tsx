  ...
  return (
    <>
      ...
      <div className="bg-white/90 rounded-3xl shadow-2xl border-4 border-yellow-400 w-full p-10 md:p-14 lg:p-16 xl:p-20 2xl:p-24 flex flex-col items-center mb-8 sm:border-yellow-400 sm:border-4 border-l-0 border-r-0">
        <ul className="mt-2 text-base sm:text-lg md:text-xl text-gray-800 font-semibold space-y-4 w-full max-w-2xl mx-auto overflow-x-auto">
          {menu.map((item, i) => {
            ...
            if (euroIndex !== -1) {
              ...
              return (
                <li key={i} className="flex items-center gap-2 md:gap-6 flex-wrap">
                  <span className="text-2xl md:text-3xl flex-shrink-0">{icon}</span>
                  <span className="text-fuchsia-800 font-bold text-lg md:text-xl truncate flex-shrink-0 max-w-[40vw]">{before}</span>
                  <span className="inline-block bg-yellow-400 text-fuchsia-900 font-extrabold rounded-full px-4 py-1 text-base md:text-lg ml-2 shadow border-2 border-yellow-600 align-middle flex-shrink-0 max-w-[30vw] truncate" style={{lineHeight: '1.8'}}>
                    {price} €
                  </span>
                </li>
              );
            ...
            return (
              <li key={i} className="flex items-center gap-2 md:gap-6 flex-wrap">
                <span className="text-2xl md:text-3xl flex-shrink-0">{icon}</span>
                <span className="text-fuchsia-800 font-bold text-lg md:text-xl truncate max-w-[60vw]">{item}</span>
              </li>
            );
          })}
        </ul>
